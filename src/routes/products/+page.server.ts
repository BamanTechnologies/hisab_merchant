...
const FETCH_PRODUCTS_QUERY = `
  query CompanyProducts($companyId: uuid!) {
    products(
...
async function fetchProducts(companyId: string | null) {
	if (!companyId) return [];
	try {
		const data = await gql<{ products: Record<string, unknown>[] }>(FETCH_PRODUCTS_QUERY, {
			companyId,
		});
...
export const load: PageServerLoad = async ({ request, parent }) => {
	...
	const [productsRaw, investors, productTypes] = await Promise.all([
	...
	const products = productsRaw.map((p) => ({
		...p,
		displayName: buildProductLabel(p as Parameters<typeof buildProductLabel>[0]),
	}));
...
export const actions: Actions = {
	createProduct: async ({ request }) => {
		...
		const productTypeName = normalizeProductTypeName(String(formData.get('product_type_name') ?? ''));
		...
		if (resolved.error || !resolved.name) {
			return { success: false, message: resolved.error ?? 'Product name is required' };
		}
		const name = resolved.name;
		if (!productTypeId) return { success: false, message: 'Product type is required' };
