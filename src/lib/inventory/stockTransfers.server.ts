...
async function fetchProductBatches(productId: string, branchId: string): Promise<StockRowForTransfer[]> {
...
async function decrementStockQuantity(stockId: string, delta: number): Promise<void> {
	...
	if (!data.update_stock_by_pk?.id) throw new Error('Failed to decrement stock');
}
...
async function insertDestinationStock(
	...
	if (!data.insert_stock_one?.id) throw new Error('Failed to create destination stock');
...
export async function executeTransfer(input: {
	...
	const { plan, fromBranch, toBranch, actorId, destinationMerchant, companyId } = input;
	...
	const transferResult = await gql<{ insert_stock_transfers_one: { id: string } | null }>(
	...
	if (batchInserts.length > 0) {
		const batchData = await gql<{ insert_stock_transfer_batches: { affected_rows: number } | null }>(
			INSERT_STOCK_TRANSFER_BATCHES_BULK,
			{ objects: batchInserts },
		);
		const affected = batchData.insert_stock_transfer_batches?.affected_rows ?? 0;
		if (affected !== batchInserts.length) {
			throw new Error(`Transfer batch insert mismatch: expected ${batchInserts.length}, got ${affected}`);
		}
	}
